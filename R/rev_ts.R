...
#improved by Hyndman, in forecast package (maybe more in tidyverts)
forecast::Acf(xxxx)
forecast::Pacf(xxxx)
...
decomp <- function(x,transform=TRUE)
{
  ...
  return(list(x=x,trend=trend,season=season,remainder=remainder,transform
              =transform,lambda=lambda))
}

# === Following function computes all measures

measuresHyndman <- function(x)
{
  require(forecast)
  ...
  # avoids the divide by zero problem by testing if the variances are
  # close to zero first
  ...
  names(m) <- c("frequency", "trend","seasonal",
                "autocorrelation","non-
                linear","skewness","kurtosis","Hurst","Lyapunov",
